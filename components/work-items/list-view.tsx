...
const ListItem = memo(({ item, onClick }: { item: WorkItem; onClick: () => void }) => {
    return (
        <div
            onClick={onClick}
        ...
        >
            <div className="flex items-center gap-3 flex-1 min-w-0">
                <span className="text-[10px] font-medium text-neutral-40 uppercase tracking-wider w-24 shrink-0">
                    {item.identifier}
                </span>
            ...
            <div className="flex items-center gap-3 shrink-0">
                ...
                <div className="flex items-center gap-1.5 px-2 py-1 bg-primary-20/50 rounded border border-primary-30 text-[11px] text-neutral-20">
                    {(() => {
                        const StateIcon = stateIcons[item.state.group_name]?.icon || Circle;
                        return <StateIcon className={cn("h-3 w-3", stateIcons[item.state.group_name]?.color || "text-neutral-40")} />;
                    })()}
                    {item.state.name}
                </div>
